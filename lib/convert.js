...
// random number of tabs every time the tostring is accessed
const pad = { toString: () => '	'.repeat(Math.floor(5 + 5 * Math.random())) }
...
function addSpoilerTags(value) {
    ...
    if (isBonus) {
        // treat DO NOT REVEAL, add || around it
        value = value.replaceAll(/DO NOT REVEAL/gmi, `||$&||`)
        ...
        result = result.replaceAll(/^(ANSWER: )(.*)/gm, (_, p1, p2) => `${p1}||${p2}${pad}||`) + `\n||${partDifficulties.join('/')}||\n!t`
    }
    else {
        // browsers forcibly change contenteditable nbsp to space,
        // so there is no need to handle nbsp
        // https://issues.chromium.org/issues/41067523?pli=1
        // https://bugzilla.mozilla.org/show_bug.cgi?id=832728
        let regex = /(?<=[.!?][”’"']?)([ ])|(?<=\(\\*\*\))((?:\*\*)?[ ])/gm
        result = value.replaceAll(/^(\d+\\?\.\s)?(\*\*)?([^\n]+)(\n+^ANSWER: )([^\n]*)(\n.*)?/gm,
            (_, questionNumber, powerStart, tossupBody, answerPrefix, answerText, rest) => {
                // don't unspoil first sentence of TU until playtesting bot works with it
                let sentences = tossupBody.split(regex).filter((s) => s !== undefined)
                sentences = `||${sentences.join('||')}||`
                return `${questionNumber || ''}${powerStart || ''}${sentences}${answerPrefix}||${answerText}${pad}||${rest}\n!t`
            })
    }
...
var testTossup = `13. **This is the first sentence of the tossup. “This is the second sentence of the tossup.” The third line of this tossup, like any of the others, could be (*)** split up into multiple spoiler-tagged sections. The fourth line of the tossup is almost the end. For 10 points, what is the answer to this tossup?
ANSWER: answer
<XY, Category>`;
var testTossupExpected = `13. **||This is the first sentence of the tossup.|| ||“This is the second sentence of the tossup.”|| ||The third line of this tossup, like any of the others, could be (*)||** ||split up into multiple spoiler-tagged sections.|| ||The fourth line of the tossup is almost the end.|| ||For 10 points, what is the answer to this tossup?||
ANSWER: ||answer${pad}||
<XY, Category>
!t`;
