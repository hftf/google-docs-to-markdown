...
function addSpoilerTags(value) {
    ...
    if (isBonus) {
        ...
        result = value.replaceAll(/^(\d+\\?\.\s)?([^\n]+)(\n+^ANSWER: )([^\n]*)(\n.*)?/gm,
            (_, questionNumber, tossupBody, answerPrefix, answerText, rest) => {
                let first = true
                const sentences = tossupBody.split(regex).map(sentence => {
                    if (first) {
                        first = false
                        return sentence
                    }
                ...
                return `${questionNumber || ''}${sentences}${answerPrefix}||${answerText}${pad}||${rest}\n!t`
            })
    }
